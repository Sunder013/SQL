...
#non - scalar or rounding functions -- round(), ceil()/ceiling(), floor(), truncate()
#round() -- if the decimal value is >=5, it rounds up to the next number,
			 #else rounds down
#ceil()/ceiling() -- always rounds up irrespective of decimal values
#floor() -- always rounds down
#truncate() -- it just trims the values
select round(288.986,-2);
select ceiling(2.1);
select floor(2.9);
select round(2.051);
select truncate(23.789, 3);
select round(avg(sales), 4) as avg_sales from superstore;
select * from my_employee;
select dep_id, round(avg(salary),1) as avg_salary from my_employee
group by dep_id order by avg_salary desc;
...
select sign(0);
select exp(2);
select log(7.38905609893065) as log_value;
select greatest(23,788,45);
select least(23,45,788);
select rand() as random_value;	#returns random value everytime when we runs, the value was between the 0 - 1


#--------------------------String--------------------------------------
#string search functions -- locate(), position(), instr()
#locate(substring, string, starting_position)
#position(substring in string) -- #string(substring)
#instr(string, substring)

select distinct order_id from superstore;
select order_id, locate('-', order_id) as  position from superstore;
select distinct customer_name, locate(' ', customer_name) as position_status from superstore;
select sistinct order_id, position('-' in order_id) from superstore;		#check this 
select distinct order_id, instr(order_id, '-') from superstore;
select distinct customer_name, position(' ' in customer_name) from superstore;
select distinct order_id, locate('-',order_id) as 1st_position,
locate('-', order_id, 4) as 2nd_position from superstore;


select distinct order_id, locate('-', order_id) as 1st_position,
locate('-', order_id, locate('-',order_id)+1) as 2nd_position from superstore;


select distinct product_name from superstore;
select distinct product_name, locate(' ',product_name) as 1st_position,
locate(' ', product_name, locate(' ', product_name)+1) as 2nd_position,
locate(' ', product_name, locate(' ', product_name, locate(' ', product_name)+1)+1) as 3rd_position from superstore;

#string concatenation function -- concat(), concat_ws()

select distinct customer_name, country from superstore;
select distinct customer_name, country, concat(customer_name, '-', country)
from superstore;
select distinct customer_name, country, region,
concat(customer_name, ' ', country, '-', region) as 'concat' from superstore;
select distinct customer_name, country, region,
concat_ws(' ', customer_name, country, region) as 'concat_ws()' from superstore;

#stringn length functions -- length(), char_length()
select distinct country, char_length(country) from superstore;

select distinct customer_name, char_length(customer_name) from superstore;

#------------------07th sep--------------------
#string modification functions -- lower(), upper(), reverse(), replace()
use sql_day_01;
select * from superstore;
select distinct customer_name, lower(customer_name) as lower_case, upper(customer_name) as upper_case from superstore;
select distinct country, lower(country) as lower_case, upper(country) as upper_case from superstore;

#reverse function -- gives existed string in reverse order
select distinct order_id, reverse(order_id) as reversed_order from superstore;
select distinct country, reverse(country) as reverse_order from superstore;

#replace function -- it will replace on string with substring
select distinct order_id, replace(order_id, 2019, 2026) as RPC from superstore;

#replace then code 'ES' with 'OS
select distinct order_id, replace(order_id, 'ES', 'OS') as RPC from superstore;


#write a query to find the length of each customer_name, reverse the customer_name, 
#change the name into lowercasea and replace the names which has 'a' letter with 's'
select distinct customer_name, char_length(customer_name),
reverse(customer_name) as reverse_order, 
lower(customer_name) as lower_case,
upper(customer_name) as upper_case,
replace(customer_name, 'a','s') as replaces_ones from superstore;


#string extraction functions - left(), right(),mid()/substring()/subst()
#left(string, lengthof_substr)
#right(string, length_of_substr)
#mid(string, length_of_substr, starting_position)
select distinct order_id from superstore;
select distinct order_id, left(order_id, 2) as code,
mid(order_id, 4, 4) as year,
right(order_id, 7) as order_number from superstore;

SELECT DISTINCT customer_name, LEFT(customer_name, LOCATE(' ', customer_name)) AS f_name,
RIGHT(customer_name, LENGTH(customer_name) - LOCATE(' ', customer_name)) AS l_name
FROM superstore;


#task -2
#abcdef@gmail.com
select left ('abcdef@gmail.com', locate('@', 'abcdef@gmail.com')-1) as user_name;
...
#lpad() -- pads left to the string		lpad(str, len, p)
#rpad() -- pads right to the string		rpad(str, len, p
...
select curtime();
select year(curdate());
select year('2025-08-23');
select month(curdate());
select month('2025-08-23');
select day('2025-08-23');
select day(current_date());
select date('2025-08-23');
select makedate(2026,250);
select weekday('2026-09-11');
...
select datediff(curdate(), '2026-09-01');
...
select date_format('2026-09-08', '%a-%M-%Y');
#%a ----- short day name -- mon,tue, wed
#b% -- short month names -- 1,2,3
#%d -- day number -- 1,2,3
#%m -- month number -- 1, 2, 3
#%D -- date with suffix --- 1st, 2nd
#%M -- full month name -- january, march
#%W -- Full day name -- monday, tuesday
#%Y -- full year -- 2026, 2025
#%y -- short year -- 26,25

#2026 - sept -- 8th
select date_format(curdate(), '%a-%M-%Y');
select distinct order_date from superstore;
use sql_day_01;

#get year, month, day from order_date column
select distinct order_date, year(order_date) as YR,
month(order_date) as MNth, day(order_date) as DY from superstore;

#get month name, day name, quarter, week from order date
select monthname(order_date) as MNth,
dayname(order_date) as DY,
quarter(order_date) as QT,
week(order_date) as WK from superstore;


#calculate shipping days 
select distinct order_date, ship_date, datediff(ship_date, order_date)
as shipping_days from superstore;



#Add 10 days to the order_date column as expected_date
select distinct order_date, date_add(order_date, interval 10 day) as 
expected_date from superstore;#add_date()


#subtract 30 days from order_date as previous_month
select distinct order_date, date_sub(order_date, interval 30 day) as
previous_month from superstore;#sub_date()


#find the orders placed in 2022
select order_id, year(order_date) as YR
from superstore where year(Order_Date) = 2022;

select * from superstore;
#find the orders placed in 'october'
select order_id, monthname(order_date) as MN
from superstore where monthname(order_date) = 'october';
...
#find the orders which are shipped on the order_date
select order_id, order_date, ship_date from superstore
where datediff(ship_date, order_date) = 0;
...
#count orders by year
#format the order-date in this format: Tues-sept-26

select monthname(order_date) as YR,
count(order_id) as total_orders
from superstore group by 1;

select quarter(order_date) as QR,
count(order_id) as total_orders
from superstore group by 1;


select order_date AS ord_date,
DATE_FORMAT(order_date, '%a-%b-%y') AS format_date
FROM superstore;
...
#last insert_id() -- returns the first auto_increment _id generated by the 
						#most recent insert statement
create table stds(id int primary key auto_increment, name varchar(30));
insert into stds(name) values('sam'), ('srinu');
select * from stds;
select last_insert_id();
insert into stds(name) values('sunder'),('Janardhan'),('sindhu');

#row count() - returns no.of rows effecred by the rect dml statements
insert into stds(name) values('miles'), ('morales');
select row_count();
...
#set operators -- to combine any two queries
#-----------------------------------set operators-------------------------
#union
#union all 
#intersect
#except
create table online_orders(c_id int, c_name varchar(20));
insert into online_orders values(101, 'Shyam'), (102, 'Sunder'), (103, 'Srinu'),
(104, 'sam'), (105, 'janardhan');

create table store_orders(c_id int, c_name varchar(20));
insert into store_orders values(104, 'sam'),(105, 'Pavan'), (106, 'Siri'),
(107, 'Anusha'), (108, 'Sindhu');

select * from online_orders;
select * from store_orders;


select c_id, c_name from online_orders
union
select c_id, c_name from store_orders;

#find the all customers who placed orders in online and store_orders
select c_id, c_name from online_orders
union all
select c_id, c_name from store_orders;


#fnd the customers who placed orders both online and store
select c_id, c_name from online_orders
intersect
select c_id, c_name from store_orders;


#find the customers who placed orders only online
select c_id, c_name from online_orders
except 
select c_id, c_name from store_orders;
...
#salary > -- 15000--- good, < 15000--- bad
#if (condition, value_if_true, value_if_false
select salary, if(salary > 15000, 'good', 'bad') as status
from my_employee;

#case statements
select salary, case when salary > 15000 then 'good' else 'bad' end as status
from my_employee;

#salary > 15000 -- good, <10000 -- bad, b/w 10 - 15000 --- ok
select salary, if(salary > 15000,'good', if(salary < 10000, 'bad', 'Ok')) as
status from my_employee;


select salary, case when salary < 15000 then 'bad'
when salary > 15000 then 'good' else 'Ok' end as status
from my_employee;


#sales > 5000 -- good, <2000 --- bad, b/w 2k - 5k --- avg

select sales, case when sales > 5000 then 'good'
when sales < 2000 then 'bad' else 'avg' end as status
from superstore;

select sales, if(sales > 5000,'good', if(sales < 2000, 'bad','avg')) as status
from superstore;


#----------------------------------Joins--------------------------------------
# inner join -- returns matching records from both the tables
# full join -- returns all the records (both matching and non matching) records #from the both tables.
#cross -- returns cartesian product of both the tables
#self -- a table joins with itself
#left join -- returns all the records from the left table and only the matching ecords from right table.
#right join -- returns all the records from the right table and only the matching records from left table.
#Equi join 				#Non - equi join 				#Natural Join



create table emps(emp_id int, emp_name varchar(30), salary decimal(10, 2),
dept_id int, manager_id int);

insert into emps values(1, 'shyam', 25000, 101, 3),(2,'sunder', 28000, 102, 4),
(3, 'srinu', 33000, 101, 5), (4, 'suresh', 38000, 102, 6),
(5, 'Tanuja', 40000, 101, 7), (6, 'Praveen', 45000, null, 7),
(7, 'Pavan', 80000, 103, null);

create table depts(dept_id int, dept_name varchar(30));
insert into depts values(101, 'IT'),(102, 'Sales'), (103, 'Ceo'),(104, 'HR'),
(105, 'Marketing');
...
select e.emp_id, e.emp_name, d.dept_id, d.dept_name from emps e inner join depts d
on e.dept_id = d.dept_id;


use sql_day_01;

select e.emp_name, d.dept_name from emps e left join depts d
on e.dept_id = d.dept_id;

select e.emp_name, d.dept_name from depts d left join emps e
on e.dept_id = d.dept_id;


select e.emp_name, d.dept_name from depts d right join emps e
on e.dept_id = d.dept_id;

select e.emp_name, d.dept_name from emps e right join depts d
on e.dept_id = d.dept_id;



#full join--- combining both tables with the union 
select e.emp_name, d.dept_name from emps e left join depts d
on e.dept_id = d.dept_id
union
select e.emp_name, d.dept_name from emps e right join depts d
...
#cross join -- partition -- all possible chances from all the records from the tables
select e.emp_name, d.dept_name from emps e cross join depts d;		#all the possible partition outcomes

select e.emp_name, d.dept_name from depts d cross join emps e;


#inner_join -- to combine records from both the tables
select e.emp_name, d.dept_name from emps e inner join depts d
on e.dept_id = d.dept_id;

select e.emp_name, d.dept_name from depts d inner join emps e
on e.dept_id = d.dept_id;

#using where clause on inner join
...
#find the emps who are earning more than 40k, along with their dept names
select e.emp_name, e.salary, d.dept_name from emps e inner join depts d
on e.dept_id = d.dept_id where e.salary > 40000;


#when we want non - matching records also then we use left join -- to get all records from the emps table 
select e.emp_name, e.salary, d.dept_name from emps e left join depts d
on e.dept_id = d.dept_id where e.salary > 40000;

#find the dept with highest avg salary
select d.dept_name, avg(e.salary) as avg_salary from emps e inner join depts d
on e.dept_id = d.dept_id group by d.dept_name order by avg_salary desc limit 1;

#find highest salary in each dept
select d.dept_name, max(e.salary) as highest_salary from emps e inner join depts d
on e.dept_id = d.dept_id group by 1;

select e.emp_name, dept_name from emps e left join depts d 
on e.dept_id = d.dept_id;

select e.emp_name, dept_name from depts d right join emps e
on e.dept_id = d.dept_id;

#find only the emps, who are not working in any dept
select e.emp_name, d.dept_name from emps e left join depts d
...
#find all the depts, including those which have no emps

select d.dept_name, e.emp_name from depts d left join emps e
on e.dept_id = d.dept_id;



#find only the depts which have no emps
select d.dept_name, e.emp_name from emps e right join depts d
on e.dept_id = d.dept_id where e.emp_name IS NULL;

select d.dept_name, e.emp_name from depts d left join emps e
on e.dept_id = d.dept_id where e.emp_name IS NULL;

#count the emps in each dept
select d.dept_name, count(e.emp_id) from emps e left join depts d
on e.dept_id = d.dept_id group by 1;
